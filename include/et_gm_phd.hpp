...
class EtGmPhd {
 ...
 public:
  EtGmPhd(const GmPhdCalibrations<4u, 2u>& calibrations) : calibrations_{calibrations} {}
 ...
 private:
  ...
  Eigen::MatrixXd CalculateKalmaGain(const InputHypothesis& input_hypothesis,
    ...
    for (auto row_index = 0u; row_index < detections_number; row_index++) {
      observation_matrix.block(0u, row_index * measurement_size, state_size, measurement_size) =
  ...
  std::pair<StateSizeVector, StateSizeMatrix> CalculateUpdatedKinematic(
    ...
    for (auto index = 0u; index < detections_number; index++) {
      observation_matrix.block(index * measurement_size, 0u, measurement_size, state_size) =
          calibrations_.observation_matrix;
      observation.block(index * measurement_size, 0u, measurement_size, 1u) =
          measurements.at(input_hypothesis.associated_measurements_indices.at(index)).value;
    }
  ...
  void CalculateDistances(const std::vector<Measurement>& measurements) {
    // Clear matrix
    for (auto& row : distance_matrix_) row.clear();
    distance_matrix_.clear();

    // Allocate new matrix
    distance_matrix_ = DistanceMatrix(measurements.size());
    for (auto& row : distance_matrix_) row.resize(measurements.size());
  ...
  void PrepareInputHypothesis(const std::vector<Measurement>& measurements) {
    ...
    for (auto& ih : input_hypothesis_) ih.associated_measurements_indices.clear();

    for (auto detection_index = 0u; detection_index < cell_numbers_.size(); detection_index++)
      input_hypothesis_.at(cell_numbers_.at(detection_index))
          .associated_measurements_indices.push_back(detection_index);
  }
