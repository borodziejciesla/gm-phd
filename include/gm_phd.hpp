...
class GmPhd {
 ...
 public:
  explicit GmPhd(const GmPhdCalibrations<state_size, measurement_size>& calibrations) : calibrations_{calibrations} {}
  ...
  void MoveSensor(const SensorPoseVector& sensor_pose_delta, const SensorPoseMatrix& sensor_pose_delta_covariance) {
    ...
    std::transform(hypothesis_.begin(), hypothesis_.end(), hypothesis_.begin(),
 ...
 protected:
  struct Hypothesis {
    ...
    Hypothesis(const double w, const StateSizeVector s, const StateSizeMatrix c) : weight{w}, state{s}, covariance{c} {}

    bool operator==(const Hypothesis& arg) {
      return (weight == arg.weight) && (state == arg.state) && (covariance == arg.covariance);
    }
  ...
  struct PredictedHypothesis {
    ...
    PredictedHypothesis(const Hypothesis h, const MeasurementSizeVector pm, const MeasurementSizeMatrix im,
                        const Eigen::Matrix<double, state_size, measurement_size> kg, const StateSizeMatrix uc)
        : hypothesis{h}, predicted_measurement{pm}, innovation_matrix{im}, kalman_gain{kg}, updated_covariance{uc} {}
 ...
 private:
  void SetTimestamps(const double timestamp) {
    if (prev_timestamp_ != 0.0) time_delta = timestamp - prev_timestamp_;
  ...
  void PredictExistingTargets(void) {
    ...
    // Predict
    std::transform(
        hypothesis_.begin(), hypothesis_.end(), std::back_inserter(predicted_hypothesis_),
        [this](const Hypothesis& hypothesis) {
          ...
          const auto kalman_gain =
              hypothesis.covariance * calibrations_.observation_matrix.transpose() * innovation_covariance.inverse();
          const auto predicted_covariance =
              (StateSizeMatrix::Identity() - kalman_gain * calibrations_.observation_matrix) * hypothesis.covariance;

          return PredictedHypothesis(predicted_state, predicted_measurement, innovation_covariance, kalman_gain,
  ...
  void UpdateExistedHypothesis(void) {
    hypothesis_.clear();
    std::transform(predicted_hypothesis_.begin(), predicted_hypothesis_.end(), predicted_hypothesis_.begin(),
  ...
  void MakeMeasurementUpdate(const std::vector<Measurement>& measurements) {
    ...
    for (const auto& measurement : measurements) {
      ...
      for (const auto& predicted_hypothesis : predicted_hypothesis_) {
        const auto weight = calibrations_.pd * predicted_hypothesis.hypothesis.weight *
                            NormPdf(measurement.value, predicted_hypothesis.predicted_measurement,
                                    predicted_hypothesis.innovation_matrix);
        const auto state =
            predicted_hypothesis.hypothesis.state +
            predicted_hypothesis.kalman_gain * (measurement.value - predicted_hypothesis.predicted_measurement);
      ...
      // Correct weights
      const auto weights_sum = std::accumulate(
          new_hypothesis.begin(), new_hypothesis.end(), 0.0,
          [this](double sum, const Hypothesis& curr) { return sum + curr.weight * (1.0 - calibrations_.pd); });
      // Normalize weight
    ...
    std::transform(predicted_hypothesis_.begin(), predicted_hypothesis_.end(), std::back_inserter(hypothesis_),
                   [](const PredictedHypothesis& predicted_hypothesis) { return predicted_hypothesis.hypothesis; });
  }
